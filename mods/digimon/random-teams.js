...
const DIGIMON_TYPES = ['Air', 'Aqua', 'Battle', 'Evil', 'Filth', 'Flame', 'Holy', 'Mech', 'Nature'];

class RandomDigimonTeams extends RandomTeams {
    prepareSet(set) {
        let randomMoves = [];

        for (let i = 0; i < 3; i++) randomMoves.push(this.sampleNoReplace(set.moves));
        ...
        return {
            ...
            species: set.species, ability: Array.isArray(set.ability) ? this.sample(set.ability) : set.ability, item: '', gender: 'N',
            moves: ['Protect', set.reservedMove].concat(randomMoves),
            // natures[~~(Math.random() * natures.length)];
            nature: this.sample(['Bashful', 'Docile', 'Hardy', 'Quirky', 'Serious']),
    ...
    randomDigimonMonotypeTeam(pool) {
        ...
        const selectedType = this.sample(DIGIMON_TYPES);
	...
	randomDigimonTeam() {
        const team = [];
        const pool = DIGIMON_SETS.filter(i => i.moves.length > 1 || i.reservedMove !== undefined);
