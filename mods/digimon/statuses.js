...
let BattleStatuses = {
	...
	raindance: {
		...
		onWeatherModifyDamage(damage, attacker, defender, move) {
			if (move.type === 'Water' || move.type === 'DG-Water') {
	...
	desolateland: {
		inherit: true,
		onTryMove(target, source, effect) {
			if ((effect.type === 'Water' || effect.type === 'DG-Water') && effect.category !== 'Status') {
	...
	deltastream: {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (move && move.effectType === 'Move' && (type === 'Flying' || type === 'DG-Electric') && typeMod > 0) {
