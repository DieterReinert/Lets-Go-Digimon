export const BattleMovedex: {[k: string]: ModdedMoveData} = {
	...
	"firewall": {
		...
		type: "DG-Fire",
		target: "self",
		desc: "Priority +4. Protects user from moves. Contact: loses 1/8 max hp unless Fire/DG-Fire type.",
		...
		effect: {
			...
			onTryHit(target, source, move) {
				...
				if (source.types.includes('Fire') || source.types.includes('DG-Fire')) return null;
	...
	"holyterrain": {
		...
		basePower: 0,
		category: "Status",
		desc: "5 turns. No Volatile, +1/16 hp. DG-Fairy moves don't miss.",
		shortDesc: "5 turns. No Volatile, +1/16 hp. DG-Fairy moves don't miss.",
		id: "holyterrain",
		name: "Holy Terrain",
		num: -294,
		signature: false,
		...
		effect: {
			...
			onAccuracy(accuracy, target, source, move) {
				if (move.type === 'DG-Fairy' && source.isGrounded()) return true;
	...
	"mudsport": {
		...
		effect: {
			...
			onBasePowerPriority: 1,
			onBasePower(basePower, attacker, defender, move) {
				if (move.type === 'Electric' || move.type === 'DG-Electric') {
	...
	"electricterrain": {
		...
		effect: {
			...
			onBasePower(basePower, attacker, defender, move) {
				if ((move.type === 'Electric' || move.type === 'DG-Electric') && attacker.isGrounded() && !attacker.isSemiInvulnerable()) {
		...
		onHitField(target, source) {
			...
			for (const side of this.sides) {
				for (const pokemon of side.active) {
					...
					if (pokemon.hasType('Grass') || pokemon.hasType('DG-Grass')) {
	...
	"grassyterrain": {
		...
		effect: {
			...
			onBasePower(basePower, attacker, defender, move) {
				...
				if ((move.type === 'Grass' || move.type === 'DG-Grass') && attacker.isGrounded()) {
	...
	"raindance": {
		inherit: true,
		desc: "For 5 turns, the weather becomes Rain Dance. The damage of Water- and DG-Water-type attacks is multiplied by 1.5 and the damage of Fire- and DG-Fire-type attacks is multiplied by 0.5 during the effect. Lasts for 8 turns if the user is holding Damp Rock. Fails if the current weather is Rain Dance.",
		shortDesc: "For 5 turns, rain powers Water/DG-Water moves.",
	},
	"sunnyday": {
		inherit: true,
		desc: "For 5 turns, the weather becomes Sunny Day. The damage of Fire- and DG-Fire-type attacks is multiplied by 1.5 and the damage of Water- and DG-Water-type attacks is multiplied by 0.5 during the effect. Lasts for 8 turns if the user is holding Heat Rock. Fails if the current weather is Sunny Day.",
