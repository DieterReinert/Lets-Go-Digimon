	...
	"statusbarrier": {
		...
		condition: {
			...
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _ => _.hasAbility, 'call', _2 => _2('persistent')])) {
	...
	"safetyguard": {
		...
		condition: {
			...
			onDamage(damage, target, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _3 => _3.effectType]) === 'Move' && damage >= target.hp) {
	...
	"critcharge": {
		...
		condition: {
			onStart(target, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _4 => _4.id]) === 'zpower') {
