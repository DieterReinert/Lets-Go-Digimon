...
function writeTB() {
	FS("config/chat-plugins/teambuilder.json").write(JSON.stringify((Utility.TB ? Utility.TB : {})));
}
...
function getStat(stat, set, tier, evOverride, natureOverride) {
	...
	let template = Dex.mod(tier).getSpecies(toID(set.species));
...
function buildMenu(userid, num, tier) {
	if (!Utility.TB[userid]) return '<span style="color:red"><b>Error: </b>User "' + userid + '" not found in TeamBuilder.</span>';
	num = parseInt(num);
	let speciesName = toID(Utility.TB[userid].monArray[tier][num].species);
	let split = Utility.TB[userid].monArray[tier][num].species.split('-');
	if (split.length > 1) {
		speciesName = toID(split[0]) + '-' + speciesName.substring(toID(split[0]).length);
	}
	let output = '';
	for (let u = 0; u < Utility.TB[userid].monArray[tier].length; u++) {
		output += '<button name="send" value="/teambuilder edit ' + tier + ', ' + (u + 1) + '"><img src="//play.pokemonshowdown.com/sprites/' + (tier !== 'digimon' ? 'bw/' : 'digimon/sprites/digimon/') + toID(Utility.TB[userid].monArray[tier][u].species) + '.png" height="32" width="32"></button>&nbsp;';
	}
	if (Utility.TB[userid].monArray[tier].length !== 6) output += '<button name="send" value="/teambuilder addmon ' + tier + ', ' + Object.keys(tierDexList[tier])[0] + '"><i class="fa fa-plus"></i></button>';
	output += '<button name="send" class="button" value="/teambuilder export ' + tier + '">Export Team</button><button name="send" class="button" value="/teambuilder removemon ' + tier + ', ' + (num + 1) + '"><i class="fa fa-trash"></i>Delete</button><button name="send" class="button" value="/dt ' + toID(Utility.TB[userid].monArray[tier][num].species) + ', ' + tier + '">Mon data</button><div class="setchart" style="height: 155px;">';
	output += '<div class="setcol setcol-icon"><div class="setcell-sprite"><img src="//play.pokemonshowdown.com/sprites/' + (tier !== 'digimon' ? 'bw/' : 'digimon/sprites/digimon/') + speciesName + '.png" height=80 width=80></div><div class="setcell setcell-pokemon"><label>Pok&eacute;mon</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/teambuilder monlist ' + tier + '">' + Utility.TB[userid].monArray[tier][num].species + '</button></div></div>';
	output += '<div class="setcol setcol-details"><div class="setrow"><div class="setcell setcell-details"><label>Details</label><button class="textbox setdetails" tabindex="-1" name="send" value="/teambuilder details main ' + tier + ', ' + (num + 1) + '"><span class="detailcell detailcell-first"><label>Level</label>' + Utility.TB[userid].monArray[tier][num].level + '</span><span class="detailcell"><label>Gender</label>' + (Utility.TB[userid].monArray[tier][num].gender === 'random' ? '-' : Utility.TB[userid].monArray[tier][num].gender) + '</span><span class="detailcell"><label>Happiness</label>' + Utility.TB[userid].monArray[tier][num].happiness + '</span><span class="detailcell"><label>Shiny</label>' + (Utility.TB[userid].monArray[tier][num].shiny ? 'Yes' : 'No') + '</span></button><span class="itemicon" style="background: none"></span></div></div><div class="setrow"><div class="setcell setcell-item"><label>Item</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/teambuilder itemlist ' + tier + ', ' + (num + 1) + '">' + (Utility.TB[userid].monArray[tier][num].item ? Utility.TB[userid].monArray[tier][num].item : '') + '</button></div><div class="setcell setcell-ability"><label>Ability</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/teambuilder ability ' + tier + ', blank, ' + (num + 1) + '">' + Utility.TB[userid].monArray[tier][num].ability + '</button></div></div></div>';
	output += '<div class="setcol setcol-moves"><div class="setcell"><label>Moves</label><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left;" name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Utility.TB[userid].monArray[tier][num].moves[0] ? Utility.TB[userid].monArray[tier][num].moves[0] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left;" name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Utility.TB[userid].monArray[tier][num].moves[1] ? Utility.TB[userid].monArray[tier][num].moves[1] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left;" name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Utility.TB[userid].monArray[tier][num].moves[2] ? Utility.TB[userid].monArray[tier][num].moves[2] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; " name="send" value="/teambuilder move ' + tier + ', blank, ' + (num + 1) + '">' + (Utility.TB[userid].monArray[tier][num].moves[3] ? Utility.TB[userid].monArray[tier][num].moves[3] : '') + '</button></div></div>';
	...
	for (let i = 0; i < statNames.length; i++) {
		stats[toID(statNames[i])] = getStat(toID(statNames[i]), {
			species: Utility.TB[userid].monArray[tier][num].species,
			evs: Utility.TB[userid].monArray[tier][num].evs,
			ivs: Utility.TB[userid].monArray[tier][num].ivs,
			nature: Utility.TB[userid].monArray[tier][num].nature,
			level: Utility.TB[userid].monArray[tier][num].level,
		}, tier);
		let evBuf = '<em>' + (Utility.TB[userid].monArray[tier][num].evs[toID(statNames[i])] === 0 ? '' : Utility.TB[userid].monArray[tier][num].evs[toID(statNames[i])]) + '</em>';
		if (Dex.mod(tier).getNature(Utility.TB[userid].monArray[tier][num].nature).plus === toID(statNames[i])) {
			evBuf += '<small>+</small>';
		} else if (Dex.mod(tier).getNature(Utility.TB[userid].monArray[tier][num].nature).minus === toID(statNames[i])) {
			evBuf += '<small>&minus;</small>';
		}
		let width = stats[toID(statNames[i])] * 75 / 504;
		if (statNames[i] === 'HP') width = stats[toID(statNames[i])] * 75 / 704;
		if (width > 75) width = 75;
		let color = Math.floor(Utility.TB[userid].monArray[tier][num].evs[toID(statNames[i])] * 180 / 714);
...
function moveMenu(userid, num, tier) {
	num = parseInt(num);
	let output = '';
	output += '<div class="setchart" style="text-align:center; height: 150px;"><h3><u>Move Menu</u><br /><button name="send" class="button" value="/dt ' + toID(Utility.TB[userid].monArray[tier][num].species) + ',' + tier + '">Mon data</button></h3><div style="padding-bottom: 2px">To remove a move click the button with the move name<br /><i>Current Moves:</i> ';
	for (let i = 0; i < Utility.TB[userid].monArray[tier][num].moves.length; i++) {
		if (Utility.TB[userid].monArray[tier][num].moves.length === 0) break;
		output += '<button name="send" class="button" value="/teambuilder removemove ' + tier + ', ' + Utility.TB[userid].monArray[tier][num].moves[i] + ', ' + (num + 1) + '">' + Utility.TB[userid].monArray[tier][num].moves[i] + '</button> , ';
	}
...
function abilityMenu(userid, num, tier) {
	num = parseInt(num);
	let output = '<div class="setchart" style="text-align:center"><h3><u>Ability Menu</u></h3><div style="padding-bottom: 2px"><i>Current Ability:</i> ' + Utility.TB[userid].monArray[tier][num].ability + '</div><div style="padding-bottom: 2px"></div>';
	let pokemon = Dex.mod(tier).getSpecies(Utility.TB[userid].monArray[tier][num].species);
...
class TeamBuilder {
	...
	species(mon, monSlot, tier) {
		...
		for (let p in dex) {
			if (dex[p].num < -200 && toID(dex[p].name).includes('pokestar') || dex[p].name === 'Missingno.') continue;
			let pokemon = dex[p];
			if (toID(pokemon.name) === speciesId) {
				this.monArray[tier][monSlot].species = pokemon.name;
				this.monArray[tier][monSlot].name = pokemon.name;
			}
		}
	...
	async addMove(move, self, monSlot, tier) {
		...
		if (Object.keys(Dex.mod(tier).getLearnsetData(toID(this.monArray[tier][monSlot].species)).learnset).indexOf(move.id) === -1) return self.errorReply(this.monArray[tier][monSlot].species + ' cannot learn ' + move.name + '.');
	...
	getMonArray(slot, userid, tier) {
		if (!Utility.TB[userid].monArray[tier][slot - 1]) return "That slot does not have a mon!";
		return slot - 1;
	}
}
...
try {
	...
	for (let first in raw) {
		for (let u in raw[first].monArray) {
			Utility.TB[first] = new TeamBuilder(first, u);
		}
		for (let second in Utility.TB[first]) {
			Utility.TB[first][second] = raw[first][second];
		}
	}
} catch (e) {
	...
	teambuilder: {
		...
		edit(target, room, user) {
			...
			if (!Utility.TB[user.id].monArray[tier]) {
				...
				Utility.TB[user.id].monArray[tier] = [{
					gender: 'random',
					level: 100,
					happiness: 160,
					item: false,
			...
			if (slot === 0) ++slot;
			let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			Utility.TB[user.id].lastMonNum = num;
			writeTB();
		...
		monlist(target, room, user) {
			...
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Utility.TB[user.id].lastMonNum, tier)}<br />${tier[0].toUpperCase() + tier.slice(1)} Pokemon list: <br />${monListDisplay}`);
		},
		...
		abilityinfo(target, room, user) {
			...
			if (!tiersList.includes(tier)) return this.errorReply(`${toID(tier)} is not a custom teambuilder tier. Available tiers: ${tiersList.join(',')}`);
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${abilityMenu(user.id, Utility.TB[user.id].lastMonNum, tier)}<br />Ability ${ability}: ${Dex.mod(tier).getAbility(ability).desc}`);
		},
		...
		itemlist(target, room, user) {
			...
			let items = '';
			if (FS('../../.data-dist/mods/' + tier + '/items').readIfExistsSync()) items = require('../../.data-dist/mods/' + tier + '/items').BattleItems;
			...
			itemDisplay += '</table>';
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Utility.TB[user.id].lastMonNum, tier)}<br />${tier[0].toUpperCase() + tier.slice(1)} Item list: <br />${itemDisplay}`);
		},
		...
		addmon(target, room, user, connection, cmd) {
			...
			if (Utility.TB[user.id].monArray[tier].length === 6) return this.errorReply(`You already have 6 pokemon!`);
			Utility.TB[user.id].monArray[tier][Utility.TB[user.id].monArray[tier].length] = {
				gender: 'random',
				level: 100,
				happiness: 160,
				item: false,
			...
			Utility.TB[user.id].lastMonNum = Utility.TB[user.id].monArray[tier].length - 1;
		   writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Utility.TB[user.id].monArray[tier].length - 1, tier)}`);
		},
		...
		species(target, room, user) {
			...
			let num = Utility.TB[user.id].getMonArray(parseInt(slot), user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			Utility.TB[user.id].species(pkmn, num, tier);
			Utility.TB[user.id].lastMonNum = num;
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, num, tier)}`);
		},
		...
		removemon(target, room, user) {
			...
			let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			if (Utility.TB[user.id].monArray[tier].length === 1) return this.errorReply(`You need more than 1 pokemon to remove a pokemon!`);
			Utility.TB[user.id].monArray[tier].splice(num, 1);
			let next = num - 1;
			if (num === 0 && Utility.TB[user.id].monArray[tier].length !== 1) next = num + 1;
			if (num === 0 && Utility.TB[user.id].monArray[tier].length === 1) next = num;
			Utility.TB[user.id].lastMonNum = next;
			writeTB();
		...
		stats: {
			...
			ev(target, room, user) {
				...
				let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Utility.TB[user.id].evs(stat, evs, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${statMenu(user.id, num, tier)}`);
			},
			...
			iv(target, room, user) {
				...
				let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Utility.TB[user.id].ivs(stat, ivs, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${statMenu(user.id, num, tier)}`);
			},
			...
			nature(target, room, user) {
				...
				let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Utility.TB[user.id].nature(nature, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${statMenu(user.id, num, tier)}`);
			},
		...
		ability(target, room, user) {
			...
			Utility.TB[user.id].ability(ability, num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${abilityMenu(user.id, num, tier)}`);
		},
		...
		move(target, room, user) {
			...
			let learnset = Dex.mod(tier).getLearnsetData(toID(Utility.TB[user.id].monArray[tier][num].species)).learnset;
			if (!learnset) return this.errorReply(`${toID(Utility.TB[user.id].monArray[tier][num].species)} does not seem to have a learnset. Make sure there is a learnset file for this format and includes every mon.`);
			learnset = Object.keys(learnset).sort();
			for (let u of learnset) {
				display += '<tr><td>' + Dex.mod(tier).getMove(u).name + '</td><td>' + Dex.mod(tier).getMove(u).shortDesc + '</td><td><button name="send" class="button" value="/teambuilder move ' + tier + ', ' + Dex.mod(tier).getMove(u).id + ', ' + (num + 1) + '">Add to ' + Utility.TB[user.id].monArray[tier][num].species + '</button></td></tr>';
			}
			display += '</table>';
			if (toID(move) === 'blank') return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${moveMenu(user.id, num, tier)}<br />${display}`);
			Utility.TB[user.id].addMove(move, this, num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${moveMenu(user.id, num, tier)}<br />${display}`);
		},
		...
		removemove(target, room, user) {
			...
			learnset = Object.keys(learnset).sort();
			for (let u of learnset) {
				display += 'Move: ' + Dex.mod(tier).getMove(u).name + ' Description: ' + Dex.mod(tier).getMove(u).shortDesc + ' <button name="send" class="button" value="/teambuilder move ' + tier + ', ' + Dex.mod(tier).getMove(u).id + ', ' + (num + 1) + '">Add to ' + Utility.TB[user.id].monArray[tier][num].species + '</button><br /><br />';
			}
			Utility.TB[user.id].removeMove(toID(move), num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${moveMenu(user.id, num, tier)}<br />${display}`);
		},
		...
		item(target, room, user) {
			...
			let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
			if (isNaN(num)) return this.errorReply(num);
			Utility.TB[user.id].item(toID(item), num, tier);
			writeTB();
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, num, tier)}`);
		},
		...
		details: {
			...
			gender(target, room, user) {
				...
				let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Utility.TB[user.id].gender(gender, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${detailMenu(user.id, num, tier)}`);
			},
			...
			level(target, room, user) {
				...
				let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Utility.TB[user.id].level(level, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${detailMenu(user.id, num, tier)}`);
			},
			...
			happiness(target, room, user) {
				...
				let num = Utility.TB[user.id].getMonArray(slot, user.id, tier);
				if (isNaN(num)) return this.errorReply(num);
				Utility.TB[user.id].happiness(hap, num, tier);
				writeTB();
				return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${detailMenu(user.id, num, tier)}`);
			},
		...
		export(target, room, user) {
			...
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuiler\n|pagehtml|${buildMenu(user.id, Utility.TB[user.id].lastMonNum, tier)}<br /><strong>Teambuilder team</strong>:<br/><br/><div style="width: 500px; overflow-x: scroll;" class="textbox chartinput">${Dex.packTeam(Utility.TB[user.id].monArray[tier])}</div><br/><br/>Paste the above into the import team section in teambuilder.`);
		},
		'': 'help',
