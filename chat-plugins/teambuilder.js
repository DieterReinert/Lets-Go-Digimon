...
function abilityMenu(userid, num, tier) {
	...
	let pokemon = Dex.mod(tier).getTemplate(Utility.TB[userid].monArray[tier][num].species);
...
exports.commands = {
	...
	teambuilder: {
		...
		itemlist(target, room, user) {
			...
			itemDisplay += '</table>';
			return user.send(`>view-teambuilder\n|init|html\n|title|${tier[0].toUpperCase() + tier.slice(1)} Teambuilder\n|pagehtml|${buildMenu(user.id, Utility.TB[user.id].lastMonNum, tier)}<br />${tier[0].toUpperCase() + tier.slice(1)} Item list: <br />${itemList.sort().join('<br />')}`);
		},
		...
		move(target, room, user) {
			...
			let learnset = Dex.mod(tier).getTemplate(toID(Utility.TB[user.id].monArray[tier][num].species)).learnset;
